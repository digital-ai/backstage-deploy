...
export function DeployResponseErrorPanel(props: DeployErrorPanelProps) {
  ...
  if (error.name === 'ResponseError') {
    ...
    if (jsonString.includes('ECONNREFUSED')) {
      ...
      title =
        'Permission Denied: The configured Deploy User lacks necessary permission for report#view in Digital.ai Deploy';
    }
    return <WarningPanel title={title ?? messageString} />;
  }
